...
int UartPsIntrExample(INTC *IntcInstPtr, XUartPs *UartInstPtr, u16 DeviceId,
  ...
  // local loop mode
  mode = XUARTPS_OPER_MODE_LOCAL_LOOP;
  XUartPs_SetOperMode(UartInstPtr, mode);
  ...
  mode = XUARTPS_OPER_MODE_NORMAL;
  XUartPs_SetOperMode(UartInstPtr, mode);

  // receive laser data
  XUartPs_Recv(UartInstPtr, laser_buffer, LASER_BUF_SIZE);
...
void Handler(void *CallBackRef, u32 Event, unsigned int EventData) {
  ...
  if (Event == XUARTPS_EVENT_RECV_DATA) {
    if (mode == XUARTPS_OPER_MODE_LOCAL_LOOP) {
      //   local loop mode
...
int SetupInterruptSystem(INTC *IntcInstancePtr, XUartPs *UartInstancePtr,
  ...
  Status = XIntc_Connect(IntcInstancePtr, UartIntrId,
                         (XInterruptHandler)XUartPs_InterruptHandler,
                         UartInstancePtr);
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
...
int UartCheck() {
  ...
  Status = UartPsIntrExample(&InterruptController, &UartPs, UART_DEVICE_ID,
                             UART_INT_IRQ_ID);
  return Status;
}
